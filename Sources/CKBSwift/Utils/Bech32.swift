...
public enum BECH32_CONST_Type: UInt32 {
case BECH32_CONST = 1
case BECH32M_CONST = 0x2bc830a3
}
/// Bech32 checksum implementation
public class CKBBech32 {
    ...
    /// Verify checksum
    private func verifyChecksum(hrp: String, checksum: Data, type: BECH32_CONST_Type = .BECH32_CONST) -> Bool {
        var data = expandHrp(hrp)
        data.append(checksum)
        return polymod(data) == type.rawValue
    }
    
    /// Create checksum
    private func createChecksum(hrp: String, values: Data, type: BECH32_CONST_Type = .BECH32_CONST) -> Data {
        var enc = expandHrp(hrp)
        enc.append(values)
        enc.append(Data(repeating: 0x00, count: 6))
        let mod: UInt32 = polymod(enc) ^ type.rawValue
    ...
    public func encode(_ hrp: String, values: Data, type: BECH32_CONST_Type = .BECH32_CONST) -> String {
        let checksum = createChecksum(hrp: hrp, values: values, type: type)
        var combined = values
    ...
    public func decode(_ str: String, type: BECH32_CONST_Type = .BECH32_CONST) throws -> (hrp: String, checksum: Data) {
        guard let strBytes = str.data(using: .utf8) else {
            throw DecodingError.nonUTF8String
        }
        guard strBytes.count <= 97 else {
        ...
        guard verifyChecksum(hrp: hrp, checksum: values, type: type) else {
